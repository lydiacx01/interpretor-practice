...
'定义一个完整的pascal语法分析器，处理：program头部、var定义、begin-end.的复杂语句 + 区分var定义的变量和计算的变量存储区域(SymbolTable和GLOBAL_MEMORY) + procedure块'
...
class ProcedureDecl(AST):
    def __init__(self, name, block):
        ...
        self.block = block
    def __str__(self):
        return '(ProcedureDecl[name={name}, block={block}])'.format(
            name = self.name,
...
class Parser(object):
    ...
    def _declaration(self):
        ...
        if (self.__currentToken.type == PROCEDURE):
            ...
            self._eat(SEMI)
            bl = self._block()
            res.append(ProcedureDecl(name, bl))
            self._eat(SEMI)
        return res
...
class SymbolTable(object):
    ...
    def __str__(self):
        return '(SymbolTable: [{t}])'.format(
            t = [repr(n) + '=' + repr(ty) for n, ty in self.__table.items()]
        )
    __repr__ = __str__
...
class SymbolBuilder(NodeVisitor):
    ...
    def _visitVarDecl(self, node):
        ...
        if (self.__symbolTable.lookup(name) is not None):
            raise Exception('Semantic error: duplicated declaration of variale: %s' % name)
        nodeType = node.type.value
        symbolType = self.__symbolTable.lookup(nodeType)
        self.__symbolTable.define(VarSymbol(name, symbolType))

    def _visitAssign(self, node):
        varName = node.left.value
        varSymbol = self.__symbolTable.lookup(varName)
    ...
    def _visitVariable(self, node):
        name = node.value
        symbol = self.__symbolTable.lookup(name)
        if (symbol is None):
            raise NameError(repr(name))
        else:
...
def test():
    while True:
        ...
        itp = SymbolBuilder()
        itp.visit(ast)
        print('done! 此时的GLOBAL_MEMORY={t}, now the symbolTable={s}'.format(
            t=itp.GLOBAL_MEMORY,
            s = repr(itp.symbolTable)
        ))
