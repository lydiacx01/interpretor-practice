...
'定义一个完整的pascal语法分析器，处理：program头部、var定义、begin-end.的复杂语句'
...
class Interpretor(object):
    ...
    def program(self, node):
        self._runBlock(node.block)

    def _runBlock(self, node):
        for n in node.vardecl:
            self._run(n)
        self._run(node.compound)

    def _calcVarDecl(self, node):
        pass
