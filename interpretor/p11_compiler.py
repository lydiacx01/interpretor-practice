...
'定义一个完整的pascal语法分析器，处理：program头部、var定义、begin-end.的复杂语句 + 区分var定义的变量和计算的变量存储区域(SymbolTable和GLOBAL_MEMORY)'
...
class Block(AST):
    def __init__(self, declarations, compound):
        self.vardecl = declarations
        self.compound = compound
    def __str__(self):
       return '(Block(vardecl={v}, compound={c}))'.format(
            v = repr(self.vardecl),
...
class Parser(object):
    ...
    def _block(self):
        vardecl = self._varDecl()
        ast = self._compound()
        self._eat(DOT)
        return Block(vardecl, ast)
